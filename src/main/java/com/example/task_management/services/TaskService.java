...
public class TaskService {
  ...
  public void createTask(TaskRequestDto request) {
    var priority =
        request.getTaskPriority() == null ? TaskPriority.NULL : request.getTaskPriority();
  ...
  private Task mapTaskEtyToTaskResponse(TaskEty taskEty) {
    ...
    List<TaskComponent> subTaskList = subTaskEtyList.stream()
        .map(entity -> (TaskComponent) SubTask.builder()
            ...
            .build())
        .toList();
    long completed = subTaskEtyList.stream()
        .filter(subtask -> subtask.getIsDone())
        .count();
    long remained = subTaskEtyList.stream()
        .filter(subtask -> !subtask.getIsDone())
        .count();
    Task task = Task.builder()
        ...
        .completedSubtasks(Integer.parseInt(Long.toString(completed)))
        .remainSubtasks(Integer.parseInt(Long.toString(remained)))
        .build();
    if (taskEty.getPriority() != TaskPriority.NULL) {
  ...
  public List<SubTask> getFilteredSubTaskList(FilterSubTaskRequest request, Integer taskId) {
    ...
    return filterStrategy.getFilteredList(taskId);
  }
  ...
  public void deleteSubTask(Integer subTaskId) {
    if (subTaskRepository.existsById(subTaskId)) {
      ...
      throw new IllegalArgumentException(
          String.format("Task with id: %d does not exists", subTaskId));
    }
  }

  public List<SubTask> getSubTaskById(Integer taskId) {
    var taskEty = taskRepository.findById(taskId).orElseThrow();
    var subtaskIterator = taskEty.getDefaultSubtaskIterator();
    List<SubTask> subTaskList = new ArrayList<>();
    while (subtaskIterator.hasNext()) {
      subTaskList.add(mapSubTaskEtyToSubTaskResponse(subtaskIterator.getNext()));
    }
  ...
  public void cloneTask(Integer taskId) {
    var taskEty = taskRepository.findById(taskId).orElseThrow();
    var clonedEntity = taskEty.clone();
    taskRepository.save(clonedEntity);
    var iterator = taskEty.getDefaultSubtaskIterator();
    List<SubTaskEty> subTaskEtyList = new ArrayList<>();
    while (iterator.hasNext()){
      var subtask = iterator.getNext().clone();
      subtask.setTask(clonedEntity);
      subTaskEtyList.add(subtask);
    }
    subTaskRepository.saveAll(subTaskEtyList);
